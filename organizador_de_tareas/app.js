// ─── SmartParser — pure Regex + Chrono.js, zero external dependencies ─────────
const SmartParser = {
  splitTasks(input) {
    const text = input.replace(/\s{2,}/g, ' ').trim();
    // Protect list-like noun sequences from splitting ("leite, pão e ovos")
    const lists = [];
    const protected_ = text.replace(
      /\b(\w[\wÀ-ú]*(?:\s*,\s*\w[\wÀ-ú]*)+(?:\s+(?:e|y|and|ou)\s+\w[\wÀ-ú]*)?)\b/gi,
      (m) => { const i = lists.length; lists.push(m); return `__L${i}__`; }
    );
    // Split on task-separating conjunctions (only before action verbs)
    const actionVerbs = '(?:também|fazer|entregar|estudar|ligar|ir|comprar|reunião|revisar|terminar|enviar|ler|escrever|agendar|marcar|finalizar|completar|preparar|also|además|equal|también|remember|lembrar|research|pesquisar|check|conferir)';
    const sep = new RegExp(`\\s*(?:;\\s*|\\s+(?:e\\s+)?(?:também|también|also|além\\s+disso|además)\\s+(?=${actionVerbs}\\b)|\\s+e\\s+(?=${actionVerbs}\\b))`, 'gi');
    const raw = protected_.split(sep).map(p => p.trim()).filter(Boolean);
  ...
  extractDateTime(text, lang) {
    ...
    const startD  = r.start.date();
    if (!hasTime) startD.setHours(23, 59, 0, 0);
    let endISO = null;
    if (r.end) endISO = r.end.date().toISOString();
  ...
  extractPriority(text) {
    ...
    const L = /(sin\s+prisa|sem\s+pressa|baja\s+prioridad|prioridade\s+baixa|low\s+priority|quando\s+puder|quando\s+der)/i;
  ...
  extractSubtasks(text) {
    ...
    if (!m) return [];
    const parts = m[1].split(/,\s*|\s+(?:e|y|and|ou)\s+/i)
      .map(s => s.replace(/[.,;!?]+$/,'').trim())
      .filter(s => s.length>1 && s.split(' ').length<=6);
    if (parts.length < 2) return [];
    return parts.map(p => p.charAt(0).toUpperCase()+p.slice(1));
  },

  parse(input, lang) {
    const segments = this.splitTasks(input);
    const tasks = [];
    const segs = segments.length ? segments : [input.trim()];
    for (const seg of segs) {
      ...
      let title = afterPrio
        .replace(/^(também|también|also|além\s+disso|además|de|em|a|o|e|para)\s+/i,'')
        .replace(/[.,;!?]+$/,'').replace(/\s{2,}/g,' ').trim();
      title = title.charAt(0).toUpperCase()+title.slice(1);
      if (!title||title.length<2) title = seg.trim();
      tasks.push({ text:title, category:this.detectCategory(seg), priority, due_date:dateISO, due_date_has_time:hasTime, due_date_end:endISO, subtaskTexts });
    }
    return tasks;
  }
};
...
class OrganizadorDeTareas {
  ...
  setLanguage(lang){this.lang=lang;storageManager.saveLanguage(lang);this.applyTranslations();this.renderStatusFilters();this.renderCalendar();this.render();}
  applyTranslations(){
    const l=this.lang;
  ...
  isUrgent(task){if(!task.due_date||task.completed)return false;const diff=new Date(task.due_date).getTime()-Date.now();return diff>0&&diff<86400000;}
  isOverdue(task){if(!task.due_date||task.completed)return false;return new Date(task.due_date).getTime()<Date.now();}
  formatDueDate(task){
    if(!task.due_date)return'';
    const locale=this.getUserLocale(),tz=this.getUserTimezone(),d=new Date(task.due_date);
    let str=task.due_date_has_time?d.toLocaleString(locale,{dateStyle:'short',timeStyle:'short',timeZone:tz}):d.toLocaleDateString(locale,{dateStyle:'medium',timeZone:tz});
    if(task.due_date_end)str+=` → ${new Date(task.due_date_end).toLocaleTimeString(locale,{hour:'2-digit',minute:'2-digit',timeZone:tz})}`;
    return str;
  }
  getLocalDateStr(isoStr){if(!isoStr)return null;try{return new Date(isoStr).toLocaleDateString('en-CA',{timeZone:this.getUserTimezone()});}catch{return null;}}
  getLocalTimeStr(isoStr){if(!isoStr)return null;try{return new Intl.DateTimeFormat('en-GB',{hour:'2-digit',minute:'2-digit',hour12:false,timeZone:this.getUserTimezone()}).format(new Date(isoStr));}catch{return null;}}
  getLocalDay(isoStr){const s=this.getLocalDateStr(isoStr);if(!s)return null;const[y,mo,d]=s.split('-').map(Number);return{year:y,month:mo,day:d};}
  ...
  removeTaskDueDate(taskId){const task=this.tasks.find(t=>t.id===taskId);if(!task)return;delete task.due_date;delete task.due_date_has_time;delete task.due_date_end;this.saveTasks();this.renderCalendar();this.render();}
  ...
  setCalendarDayFilter(year,month,day){const f=this.calendarDayFilter;this.calendarDayFilter=(f&&f.year===year&&f.month===month&&f.day===day)?null:{year,month,day};this.renderCalendar();this.render();}
  ...
  renderCalendar(){
    const container=document.getElementById('miniCalendar');if(!container)return;
    const year=this.calendarDate.getFullYear(),month=this.calendarDate.getMonth(),locale=this.getUserLocale();
    const titleEl=document.getElementById('calMonthTitle');
    if(titleEl){const raw=new Intl.DateTimeFormat(locale,{month:'long',year:'numeric'}).format(new Date(year,month,1));titleEl.textContent=raw.charAt(0).toUpperCase()+raw.slice(1);}
    ...
    const firstDow=new Date(year,month,1).getDay(),offset=(firstDow-startDow+7)%7,daysInMonth=new Date(year,month+1,0).getDate();
    const cells=[];for(let i=0;i<offset;i++)cells.push(null);for(let d=1;d<=daysInMonth;d++)cells.push(d);while(cells.length%7!==0)cells.push(null);
    const todayLocal=this.getLocalDay(new Date().toISOString());
    const isToday=d=>d&&todayLocal&&todayLocal.year===year&&todayLocal.month===month+1&&todayLocal.day===d;
    const isSel=d=>{const f=this.calendarDayFilter;return d&&f&&f.year===year&&f.month===month+1&&f.day===d;};
    container.innerHTML=`
      <div class="grid grid-cols-7 mb-1">${headers.map(h=>`<div class="text-center text-[10px] font-bold text-amber-700/60 dark:text-amber-400/60 py-0.5">${h}</div>`).join('')}</div>
      <div class="grid grid-cols-7 gap-y-0.5">${cells.map(d=>{
        if(!d)return`<div></div>`;
        const info=dayMap.get(d),sel=isSel(d),tod=isToday(d),hasTasks=!!info;
        const cellCls=sel?'bg-blue-500 rounded-full cursor-pointer ring-2 ring-blue-300/50':tod?'bg-amber-400/80 dark:bg-amber-600/70 font-bold rounded-full cursor-pointer hover:bg-amber-500/80':hasTasks?'cursor-pointer hover:bg-amber-100/80 dark:hover:bg-zinc-700/60 rounded-md':'rounded-md opacity-60';
        const numCls=sel?'text-white':tod?'text-amber-950 dark:text-amber-50':'text-amber-900 dark:text-amber-100';
        let dots='';if(info){if(info.hasOverdue)dots+=`<span class="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0"></span>`;if(info.hasUrgent)dots+=`<span class="w-1.5 h-1.5 rounded-full bg-orange-400 shrink-0 animate-pulse"></span>`;if(info.hasNormal)dots+=`<span class="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0"></span>`;}
        const tooltip=info?`title="${info.tasks.map(t=>t.text.slice(0,40)).join('\n')}"`:'' ;
        const click=(hasTasks||tod)?`onclick="organizator.setCalendarDayFilter(${year},${month+1},${d})"`:'';
        return`<div class="flex flex-col items-center justify-center py-1 transition ${cellCls}" ${tooltip} ${click}><span class="text-[11px] leading-none font-medium ${numCls}">${d}</span><div class="flex gap-0.5 justify-center h-2 mt-0.5">${dots}</div></div>`;
      }).join('')}</div>`;
  ...
  updateCalendarFilterBadge(){
    ...
    if(this.calendarDayFilter){const f=this.calendarDayFilter;const str=new Date(f.year,f.month-1,f.day).toLocaleDateString(this.getUserLocale(),{weekday:'short',day:'numeric',month:'long'});badge.innerHTML=`<span class="font-semibold">📅 ${str}</span><button onclick="organizator.clearCalendarDayFilter()" class="ml-2 text-blue-700 dark:text-blue-300 hover:text-red-500 font-bold transition">${this.t('calClearFilter')}</button>`;badge.classList.remove('hidden');}
  ...
  setActiveTab(tab){
    ...
    ['manualTabBtn','smartTabBtn'].forEach(id=>{const btn=document.getElementById(id);if(!btn)return;const active=(id==='manualTabBtn'&&tab==='manual')||(id==='smartTabBtn'&&tab==='smart');btn.classList.toggle('active',active);btn.classList.toggle('text-amber-900',active);btn.classList.toggle('dark:text-amber-100',active);btn.classList.toggle('text-amber-700/70',!active);btn.classList.toggle('dark:text-amber-300/70',!active);});
  }
    ...
    const tasksHTML=this.smartPreviewTasks.map((t,idx)=>{
      ...
      return`<div class="rounded-xl border border-blue-200/50 dark:border-blue-700/30 bg-white/60 dark:bg-zinc-800/60 overflow-hidden">
        ...
        <div class="px-4 py-3 space-y-3">
          <input type="text" id="pt-text-${idx}" value="${this.escapeHTML(t.text)}"
            class="w-full rounded-lg border border-amber-900/20 dark:border-amber-600/30 bg-amber-50/60 dark:bg-zinc-900/70 px-3 py-2 text-sm text-amber-900 dark:text-amber-100 font-serif italic focus:outline-none focus:ring-2 focus:ring-blue-400/50 transition">
          ...
          <div class="grid grid-cols-2 gap-2">
            ...
            ${t.due_date_end?`<div class="space-y-1 col-span-2 sm:col-span-1"><label class="text-[10px] font-bold uppercase tracking-wider text-violet-700/60 dark:text-violet-400/60">Hora fim</label>
          ...
          <div class="space-y-1.5"><label class="text-[10px] font-bold uppercase tracking-wider text-blue-700/60 dark:text-blue-400/60">${this.t('previewSubtasksLabel')}</label>
            ...
            <div class="flex gap-2">
              <input type="text" id="pt-subinput-${idx}" placeholder="${this.t('addSubtaskPlaceholder')}"
                class="flex-1 rounded-lg border border-blue-200/40 dark:border-blue-700/30 bg-white/60 dark:bg-zinc-900/60 px-2.5 py-1.5 text-xs text-[#6b7280] dark:text-amber-100 focus:outline-none focus:ring-2 focus:ring-blue-400/40 transition"
                onkeypress="if(event.key==='Enter'){organizator.addPreviewSubtask(${idx});event.preventDefault();}">
    ...
    overlay.innerHTML=`<div class="fixed inset-0 bg-black/65 backdrop-blur-sm z-[200] flex items-center justify-center p-4" onclick="if(event.target===this) organizator.cancelSmartPreview()">
      <div class="relative w-full max-w-2xl bg-parchment/98 dark:bg-zinc-900/98 rounded-2xl shadow-2xl border-2 border-blue-400/40 dark:border-blue-500/35 flex flex-col max-h-[90vh]" onclick="event.stopPropagation()">
        <div class="flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-indigo-600/90 via-violet-600/80 to-fuchsia-600/70 text-white rounded-t-2xl shrink-0">
          ...
          <div class="flex-1"><h3 class="font-bold text-sm tracking-wide">${this.t('previewTitle')}</h3><p class="text-[11px] opacity-80 mt-0.5">${this.smartPreviewTasks.length} tarefa(s) detectada(s) — revise e confirme</p></div>
  ...
  removePreviewSubtask(taskIdx,subIdx){if(this.smartPreviewTasks[taskIdx])this.smartPreviewTasks[taskIdx].subtasks.splice(subIdx,1);this.openSmartPreviewModal();}
  ...
  confirmSmartTask(){
    ...
    this.smartPreviewTasks.forEach((t,idx)=>{
      ...
      const category=(document.getElementById(`pt-cat-${idx}`)?.value||t.category).trim();
  ...
  cancelSmartPreview(){this.smartPreviewTasks=[];const overlay=document.getElementById('smartPreviewOverlay');if(overlay){overlay.innerHTML='';overlay.classList.add('hidden');}document.body.style.overflow='';}

  init(){this.loadTheme();this.applyTranslations();this.renderStatusFilters();this.renderCalendar();this.bindEvents();this.render();this.setActiveTab('manual');}

  escapeHTML(str){return String(str).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));}
  getPriorityConfig(p){return({high:{textKey:'prioHigh',color:'bg-red-500/90'},medium:{textKey:'prioMedium',color:'bg-amber-500/90'},low:{textKey:'prioLow',color:'bg-emerald-500/90'}})[p]||{textKey:'prioMedium',color:'bg-amber-500/90'};}
  ...
  addSubtask(taskId){const input=document.getElementById(`subtask-input-${taskId}`);if(!input)return;const text=input.value.trim();if(!text)return;const task=this.tasks.find(t=>t.id===taskId);if(!task)return;if(!task.subtasks)task.subtasks=[];task.subtasks.push({id:Date.now().toString(),text,completed:false});this.saveTasks();input.value='';this.openSubtaskPanels.add(taskId);this.render();}
  toggleSubtask(taskId,subtaskId){const task=this.tasks.find(t=>t.id===taskId);if(!task?.subtasks)return;const sub=task.subtasks.find(s=>s.id===subtaskId);if(!sub)return;sub.completed=!sub.completed;task.completed=task.subtasks.length>0&&task.subtasks.every(s=>s.completed);this.saveTasks();this.openSubtaskPanels.add(taskId);this.render();}
  toggleSubtaskPanel(taskId){this.openSubtaskPanels.has(taskId)?this.openSubtaskPanels.delete(taskId):this.openSubtaskPanels.add(taskId);document.getElementById(`subtasks-panel-${taskId}`)?.classList.toggle('open',this.openSubtaskPanels.has(taskId));const arrow=document.getElementById(`subtask-arrow-${taskId}`);if(arrow)arrow.textContent=this.openSubtaskPanels.has(taskId)?'▲':'▼';}
  editSubtask(taskId,subtaskId){const task=this.tasks.find(t=>t.id===taskId);if(!task?.subtasks)return;const sub=task.subtasks.find(s=>s.id===subtaskId);if(!sub)return;const v=prompt(this.t('editSubtaskPrompt'),sub.text);if(v===null)return;const c=v.trim();if(c){sub.text=c;this.saveTasks();this.openSubtaskPanels.add(taskId);this.render();}}
  deleteSubtask(taskId,subtaskId){const task=this.tasks.find(t=>t.id===taskId);if(!task?.subtasks)return;if(!confirm(this.t('confirmDeleteSubtask')))return;task.subtasks=task.subtasks.filter(s=>s.id!==subtaskId);this.saveTasks();this.openSubtaskPanels.add(taskId);this.render();}

  buildProgressHTML(prog){const c=prog.pct===100?'bg-emerald-500':prog.pct>=50?'bg-amber-500':'bg-red-400';return`<div class="flex items-center gap-2"><span class="text-[10px] uppercase tracking-wider text-amber-700/70 dark:text-amber-400/70 shrink-0 hidden sm:inline">${this.t('progressLabel')}</span><div class="flex-1 h-2 rounded-full bg-amber-200/60 dark:bg-zinc-700/60 overflow-hidden min-w-[50px]"><div class="progress-fill h-full rounded-full ${c}" style="width:${prog.pct}%"></div></div><span class="text-[10px] font-bold text-amber-800 dark:text-amber-300 shrink-0">${prog.done}/${prog.total}</span></div>`;}
  buildSubtaskListHTML(task){if(!task.subtasks?.length)return'';return task.subtasks.map(sub=>`<div class="flex items-center gap-2 py-1.5 px-2 rounded-lg hover:bg-amber-100/50 dark:hover:bg-zinc-700/40 group transition"><input type="checkbox" id="sub-${sub.id}" class="h-4 w-4 rounded accent-emerald-600 cursor-pointer shrink-0" ${sub.completed?'checked':''} onchange="organizator.toggleSubtask('${task.id}','${sub.id}')"><label for="sub-${sub.id}" class="flex-1 text-sm text-amber-900/90 dark:text-amber-100/80 cursor-pointer select-none ${sub.completed?'line-through opacity-55':''}" ondblclick="organizator.editSubtask('${task.id}','${sub.id}')" title="${this.t('dblClickHint')}">${this.escapeHTML(sub.text)}</label><button class="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-600 transition text-xs shrink-0" onclick="organizator.deleteSubtask('${task.id}','${sub.id}')">✕</button></div>`).join('');}
  buildDeadlineHTML(task){
    ...
    const badgeCls=overdue?'bg-red-100 dark:bg-red-900/50 border-red-400/60 text-red-700 dark:text-red-200':urgent?'bg-orange-100 dark:bg-orange-900/50 border-orange-400/60 text-orange-700 dark:text-orange-200':'bg-amber-50 dark:bg-zinc-800/60 border-amber-900/20 dark:border-amber-500/20 text-amber-700 dark:text-amber-300';
    return`<div class="mt-2 mb-1 space-y-1.5"><div class="flex items-center gap-2 flex-wrap"><span class="text-[10px] text-amber-700/50 dark:text-amber-400/50 font-semibold select-none shrink-0">📅</span><input type="date" id="${dateId}" value="${dateVal}" style="color-scheme:light dark" class="rounded-lg border border-amber-900/20 dark:border-amber-600/25 bg-amber-50/40 dark:bg-zinc-900/50 px-2 py-1 text-xs text-[#6b7280] dark:text-amber-100/80 focus:outline-none focus:ring-2 focus:ring-amber-400/50 transition cursor-pointer" onchange="organizator.setTaskDueDate('${task.id}',this.value,document.getElementById('${timeId}')?.value||'')"><input type="time" id="${timeId}" value="${timeVal}" style="color-scheme:light dark" class="rounded-lg border border-amber-900/20 dark:border-amber-600/25 bg-amber-50/40 dark:bg-zinc-900/50 px-2 py-1 text-xs text-[#6b7280] dark:text-amber-100/80 focus:outline-none focus:ring-2 focus:ring-amber-400/50 transition cursor-pointer" onchange="organizator.setTaskDueDate('${task.id}',document.getElementById('${dateId}')?.value||'',this.value)">${hasDate?`<button onclick="organizator.removeTaskDueDate('${task.id}')" class="text-[10px] text-red-400/60 hover:text-red-500 transition font-black shrink-0">✕</button>`:''}</div>${hasDate?`<div class="flex items-center gap-2 flex-wrap">${task.due_date_end?`<span class="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-violet-200/70 dark:bg-violet-800/40 text-violet-700 dark:text-violet-300">⏱</span>`:''}<span class="inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full border ${badgeCls} ${urgent?'animate-pulse':''}">📅 ${this.formatDueDate(task)}</span>${overdue?`<span class="text-[10px] font-black text-red-600 dark:text-red-400 uppercase">❌ ${this.t('overdueBadge')}</span>`:''}${urgent?`<span class="text-[10px] font-black text-orange-500 dark:text-orange-400 uppercase animate-pulse">⚠️ ${this.t('urgentBadge')}</span>`:''}</div>`:''}</div>`;
  }
  ...
  generateTaskHTML(task){
    ...
    return`<article data-id="${task.id}" draggable="${!this.sortedByDeadline}" class="task-item cursor-move relative bg-gradient-to-br from-amber-50/80 via-amber-100/60 to-amber-50/70 dark:from-zinc-800/70 dark:via-zinc-700/60 dark:to-zinc-800/50 border border-amber-800/30 dark:border-amber-400/40 border-l-8 border-amber-900/60 dark:border-amber-300/50 shadow-[8px_8px_16px_rgba(0,0,0,0.3)] rounded-2xl p-5 pt-8 pb-4 transition-all duration-200 ${ring} ${isSel?'ring-2 ring-blue-500/60 scale-[1.01]':''}">
      ...
      <div class="flex items-center justify-between flex-wrap gap-2 mb-1">
        <p class="text-xs sm:text-sm text-amber-700/80 dark:text-amber-300/70 font-serif italic flex items-center gap-1 cursor-pointer select-none" ondblclick="organizator.editCategory('${task.id}')" title="${this.t('dblClickHint')}">📓 <span>${this.escapeHTML(task.category)}</span><span class="text-[10px] text-amber-500/35 italic">✎✎</span></p>
  ...
  renderStatusFilters(){const c=document.getElementById('statusFiltersContainer');if(!c)return;c.innerHTML=this.statusFilters.map(f=>{const on=this.currentFilter===f.id;return`<button class="filter-btn text-xs sm:text-sm font-semibold px-3 py-1.5 rounded-full border transition ${on?`border-${f.color}-900/40 bg-${f.color}-100/80 dark:bg-${f.color}-700/70 text-ink dark:text-${f.color}-50 shadow-sm`:`border-${f.color}-700/60 bg-transparent text-${f.color}-800 dark:text-${f.color}-200 hover:bg-${f.color}-100/30`}" data-filter="${f.id}">${this.t(f.labelKey)}</button>`;}).join('');}
  renderCategoryFilters(){
    const container=document.getElementById('categoryFilterContainer'),catSelect=document.getElementById('newCategorySelect');if(!container)return;
    const cats=['all'];this.tasks.forEach(t=>{if(!cats.includes(t.category))cats.push(t.category);});
    let btns='',sel='';
    cats.forEach(cat=>{const on=this.currentCategory===cat,label=cat==='all'?this.t('filterAll'):cat;const cls=cat==='all'?(on?'bg-amber-200/90 dark:bg-amber-700/80 border-amber-900/60 text-amber-900 dark:text-amber-50':'bg-amber-100/60 dark:bg-amber-800/40 border-amber-900/30 text-amber-800 dark:text-amber-200 hover:bg-amber-200/60'):(on?'bg-blue-200/90 dark:bg-blue-800/80 border-blue-600/60 text-blue-900 dark:text-blue-50':'bg-blue-50/70 dark:bg-blue-900/40 border-blue-400/50 text-blue-800 dark:text-blue-200 hover:bg-blue-100');btns+=`<button class="cat-filter-btn ${cls} text-xs font-semibold px-3 py-1.5 rounded-full border shadow-sm transition min-w-[55px]" data-category="${cat}">${label}</button>`;if(cat!=='all')sel+=`<option value="${cat}">${this.escapeHTML(cat)}</option>`;});
    sel+=`<option value="nueva">${this.t('writeNew')}</option>`;
    container.innerHTML=btns;
    if(catSelect){const cur=catSelect.value;catSelect.innerHTML=sel;if(cats.includes(cur)&&cur!=='nueva')catSelect.value=cur;else catSelect.value=cats.length>1?cats[1]:'General';}
    const ti=document.getElementById('newCategoryText'),cb=document.getElementById('cancelNewCatBtn');
    if(ti&&cb&&catSelect){ti.addEventListener('input',()=>cb.classList.toggle('hidden',ti.classList.contains('hidden')));catSelect.addEventListener('change',()=>cb.classList.toggle('hidden',ti.classList.contains('hidden')));}
  }
  render(){
    ...
    if(!filtered.length){const msg=this.calendarDayFilter?`<div class="text-center py-6 text-[#6b7280] dark:text-amber-300">📅 Sin tareas este día.<br><button class="mt-2 text-xs text-blue-600 dark:text-blue-400 underline" onclick="organizator.clearCalendarDayFilter()">Limpiar filtro</button></div>`:`<div class="text-center py-8 text-[#6b7280] dark:text-amber-300">${this.searchTerm?this.t('noTasksSearch'):this.t('noTasks')}</div>`;container.innerHTML=`<h2 class="text-xl font-semibold tracking-wide text-amber-900 dark:text-amber-100 mb-2">${this.t('taskListTitle')}</h2>${msg}`;return;}
  ...
  bindEvents(){
    ...
    document.addEventListener('keydown',e=>{if(e.key==='Escape'&&this.smartPreviewTasks.length)this.cancelSmartPreview();});
    document.addEventListener('click',e=>{if(e.target.classList.contains('filter-btn')){this.currentFilter=e.target.dataset.filter;this.selectedTasks.clear();this.renderStatusFilters();this.render();}if(e.target.classList.contains('cat-filter-btn')){this.currentCategory=e.target.dataset.category;this.selectedTasks.clear();this.render();}});
    const dnd=document.getElementById('tasksContainer');
    if(dnd){dnd.addEventListener('dragstart',e=>{if(this.sortedByDeadline)return;const it=e.target.closest('.task-item');if(!it)return;this.draggedId=it.dataset.id;setTimeout(()=>it.classList.add('opacity-40','scale-95'),0);});dnd.addEventListener('dragover',e=>e.preventDefault());dnd.addEventListener('drop',e=>{e.preventDefault();const it=e.target.closest('.task-item');if(it&&this.draggedId&&it.dataset.id!==this.draggedId)this.reorderTasks(this.draggedId,it.dataset.id);});dnd.addEventListener('dragend',e=>{e.target.closest('.task-item')?.classList.remove('opacity-40','scale-95');this.draggedId=null;});}
  }

  loadTheme(){const isDark=storageManager.loadTheme('light')==='dark';document.documentElement.classList.toggle('dark',isDark);document.documentElement.classList.toggle('light',!isDark);document.documentElement.style.colorScheme=isDark?'dark':'light';const ti=document.getElementById('themeIconFixed');if(ti)ti.textContent=isDark?'☀️':'🌙';}
  ...
  addTask(){
    const titleEl=document.getElementById('newTaskInput'),prioEl=document.getElementById('newPriorityInput'),catSel=document.getElementById('newCategorySelect'),catText=document.getElementById('newCategoryText'),dateEl=document.getElementById('newTaskDate'),timeEl=document.getElementById('newTaskTime'),cancelBtn=document.getElementById('cancelNewCatBtn');
    const text=titleEl?.value.trim();if(!text){titleEl?.focus();return;}
    const priority=prioEl?.value||'medium';
    let category='General';if(catText&&!catText.classList.contains('hidden')&&catText.value.trim())category=catText.value.trim();else if(catSel?.value&&catSel.value!=='nueva')category=catSel.value;
    ...
    if(titleEl)titleEl.value='';if(prioEl)prioEl.value='medium';if(dateEl)dateEl.value='';if(timeEl)timeEl.value='';if(catText){catText.value='';catText.classList.add('hidden');}if(cancelBtn)cancelBtn.classList.add('hidden');if(catSel)catSel.classList.remove('hidden');
  ...
  toggleTask(taskId){const task=this.tasks.find(t=>t.id===taskId);if(!task)return;task.completed=!task.completed;if(task.subtasks?.length)task.subtasks.forEach(s=>s.completed=task.completed);this.saveTasks();this.render();}
  ...
  updateBulkActionUI(){const sb=document.getElementById('selectAllBtn'),cb=document.getElementById('completeActionBtn'),db=document.getElementById('deleteActionBtn');if(!sb||!cb||!db)return;const f=this.tasks.filter(t=>this.matchesFilter(t));sb.innerHTML=f.length>0&&f.every(t=>this.selectedTasks.has(t.id))?this.t('deselectAll'):this.t('selectAll');const targets=this.selectedTasks.size>0?this.tasks.filter(t=>this.selectedTasks.has(t.id)):f;const allDone=targets.length>0&&targets.every(t=>t.completed);if(this.selectedTasks.size>0){cb.innerHTML=allDone?`${this.t('pendingSelected')} (${this.selectedTasks.size})`:`${this.t('completeSelected')} (${this.selectedTasks.size})`;db.innerHTML=`${this.t('deleteSel')} (${this.selectedTasks.size})`;}else{cb.innerHTML=(allDone&&f.length>0)?this.t('pendingAll'):this.t('completeAll');db.innerHTML=this.t('deleteDone');}}
  matchesFilter(t){const ms=t.text.toLowerCase().includes(this.searchTerm)||t.category.toLowerCase().includes(this.searchTerm);const mp=this.currentFilter==='all'||this.currentFilter===t.priority||(this.currentFilter==='completed'&&t.completed)||(this.currentFilter==='pending'&&!t.completed);const mc=this.currentCategory==='all'||t.category===this.currentCategory;let md=true;if(this.calendarDayFilter){if(!t.due_date){md=false;}else{const ld=this.getLocalDay(t.due_date),f=this.calendarDayFilter;md=ld&&ld.year===f.year&&ld.month===f.month&&ld.day===f.day;}}return ms&&mp&&mc&&md;}
  updateStats(){const total=this.tasks.length,done=this.tasks.filter(t=>t.completed).length;const te=document.getElementById('statTotal'),de=document.getElementById('statCompleted'),pe=document.getElementById('statPending');if(te)te.textContent=String(total);if(de)de.textContent=String(done);if(pe)pe.textContent=String(total-done);}
